...
bool CAutoWall::CanPenetrate(SDK::CBaseEntity* attacker, Autowall_Info &info, SDK::CSWeaponInfo* weapon_data)
{
	...
	is_autowalling = true;
	bool return_value = !HBP(attacker, weapon_data->penetration, material, &use_static_values, &info.enter_trace, &info.direction, 0.f, enter_surface_data->game.flPenetrationModifier, enter_surface_data->game.flDamageModifier, 0, mask, weapon_data->penetration, &info.penetration_count, &info.current_position, 0.f, 0.f, &info.current_damage);
