...
void skinchanger()
{
	...
	for (size_t i = 0; weapons[i] != INVALID_EHANDLE_INDEX; i++)
	{
		...
		if (pEntity)
		{
			...
			{
				int Model = SETTINGS::settings.skinchanger_knifemodel;
				int weapon = *pWeapon->fixskins();
				switch (weapon)
				{
					// we don't need weapons for now, i'm too lazy to make the menu things
				/* case 7: // AK47 
				{
					switch (SETTINGS::settings.AK47)
					{
					case 0:
				...
				*/
				...
				if (pEntity->GetClientClass()->m_ClassID == (int)93)
				{
					...
					if (Model == 0) // Bayonet
					{
						int iBayonet = INTERFACES::ModelInfo->GetModelIndex("models/weapons/w_minecraft_pickaxe.mdl");
						...
						*pWeapon->GetEntityQuality() = 3;
						...
						int Skin = SETTINGS::settings.skinchanger_knifeskin;
						if (Skin == 0)
						{
